...
function buildPackages(destDocs) {
  const pkgs = [];
  const titles = [
    "Romantic Escape", "Family Fun", "Backpacker Special", "Luxury Retreat",
    "Adventure Trail", "Honeymoon Bliss", "Weekend Getaway", "Grand Tour",
  ];
  let i = 0;
  while (pkgs.length < 20) {
    const dest = destDocs[i % destDocs.length];
    const titleBase = titles[pkgs.length % titles.length];
    const days = 4 + (pkgs.length % 7); // 4..10
...
const seed = async () => {
  await connectDB();
  console.log("Clearing existing destinations & packages...");
  await Destination.deleteMany({});
  await Package.deleteMany({});

  const withSlugs = destinations.map((d) => ({ ...d, slug: slugify(`${d.name}-${d.country}`) }));
  const destDocs = await Destination.insertMany(withSlugs);
  console.log(`Inserted ${destDocs.length} destinations`);

  const pkgs = buildPackages(destDocs);
  const pkgDocs = await Package.insertMany(pkgs);
  console.log(`Inserted ${pkgDocs.length} packages`);

  // Seed an admin + demo user if none exist
